...
upversion() {
  mvn --file "${BASE}/code/format/pom.xml" versions:set -DnewVersion="$1"
}

echo "Test current state"

mvn --file "${BASE}/code/format/pom.xml" clean package -P release
...
mvn --file "${BASE}/code/format/pom.xml" clean deploy -P release
